from duplicated_info_finder import similarity, similarity_numeric
...
def from_db_to_emergency(document_title, driver=db_driver):
    ...
    location_node = session.run("MATCH (n:Document {attrs})-[:HAS_LOCATION]->(i) return i".format(attrs="{title: \"" + document_title + "\"}"))
    ...
    contaminants_nodes = session.run("MATCH (n:Document {attrs})-[:HAS_CONTAMINANT]->(i)-[:HAS_SYMPTOMS]->(s) return i,s".format(attrs="{title: \"" + document_title + "\"}"))
    ...
    for contaminant in contaminants_names:
        contaminants.append(Contaminant(contaminant,[elem["s"]["name"] for elem in contaminants_data if elem["i"]["name"] == contaminant]))
    ...
    sources_node = session.run("MATCH (n:Document {attrs})-[:HAS_SOURCE]->(i) return i".format(attrs="{title: \"" + document_title + "\"}"))
    sources_data = sources_node.data()
    sources = [elem["i"]["description"] for elem in sources_data]
    causes_node = session.run("MATCH (n:Document {attrs})-[:HAS_CAUSE]->(i) return i".format(attrs="{title: \"" + document_title + "\"}"))
