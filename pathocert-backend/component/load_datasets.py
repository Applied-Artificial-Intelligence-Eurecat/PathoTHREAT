...
def load_dataset(file):
    ...
    try:
        ...
        print(f"{Fore.RED}FILE {file} NOT FOUND!\nDid you preprocess the dataset?{Style.RESET_ALL}")
        raise FileNotFoundError
    return data


def load_datasets(filenames=('data.tsv', 'test.tsv')):
    csv.register_dialect('tsv', delimiter='\t')

    try:
        train_dict, test_dict = load_dataset(filenames[0]), load_dataset(filenames[1])
    except FileNotFoundError:
        return None
